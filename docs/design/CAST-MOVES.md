...
**Status:** SPEC, for PB comment (agreed process, deliberation 2026-07-10) then
Cx review then build. Task #80 — "the single highest-leverage remaining gap"
...
Partition the resolved narrator rows once more: rows with
`attribute == "in"` whose ENTITY is a canon **person** (folded kind) leave the
ordinary promote flow and enter the cast-moves lane. Everything else is
untouched (objects keep the held-object guard and ordinary promotion; places
keep their no-mint channel policy).
...
3. **The destination BINDS.** `place:D` must already exist in canon (the
   ...
   of this upstream; the lane re-verifies kind==place on the bound id.
4. **The move touches the CURRENT scene.** Either origin (X's current `locate`
   head) or destination is the scene (an on-screen departure or arrival). A
...
valid_from=turn_time(turn))` **direct to canon** — NOT through the
proposed-frame quarantine. Rationale: the move was already RENDERED to the
player as world truth; staging it for a gate that exists to catch contradictions
would re-create the render/truth drift this lane exists to close. The
contradiction the promote gate would flag (the old `in` value) is precisely the
supersession the move intends; single-parent semantics retire the old holder
automatically (PB 125: "no delete, no cleanup"). `classify="rules"` = zero
model calls (an `in` row on a person is unambiguous STATE).
...
- A committed DEPARTURE additionally writes the existing `departed_scene`
  ...
  reusing the shipped mechanism rather than inventing a second one. An arrival
  needs no event: the `in` row IS presence-positive truth.
...
2. **Departure:** narrated exit (origin==scene) → committed + `departed_scene`
   event; X gone from presence next turn; no re-offer.
...
9. **Ordinary person rows** (non-`in`) still flow through the normal promote
   gate unchanged — the lane takes ONLY movement.
...
PB comment round on this doc (the extract-contract seam is where host specs
drift from engine truth — PB 125's own advice) → Cx spec review → build
