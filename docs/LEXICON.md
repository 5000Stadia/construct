...
| **refusal clock** | The per-arc REFUSAL-rung clock, now an **explicit-abandonment** `Occurred(event:abandoned_<arc>)` condition (founder ruling 2026-06-25 / Cx 176/178): it fires ONLY when the player decisively walks away — NEVER on quiet turns. Turns never force a close (a counter-based refusal is a lint error and is suppressed at runtime). Stories conclude on the narrative's decisive event: a player commitment, an authored deadline/loss, or explicit abandonment. | Plain. |
