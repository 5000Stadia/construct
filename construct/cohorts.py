...
IMAGE_PROMPT_SCHEMA = {
    ...
    "properties": {
        "prompt": {"type": "string", "description": "A single vivid text-to-image "
            "prompt for the LOCATION — but depicting ONLY what the description actually "
            "states. Render the architecture, surfaces, materials, light, weather, "
            "colour and mood, and EXACTLY the objects/features the description names "
            "(if it mentions a slain body, a broken wagon, an overturned chair, spilled "
            "blood — keep those; they are part of the scene). DO NOT INVENT objects the "
            "description doesn't mention — especially NO discrete or interactable items "
            "a player might try to use (no keys, weapons, daggers, letters, documents, "
            "coins, jewellery, bottles, open books, footprints) unless the text names "
            "them: the picture must show only what is truly in the room, or the player "
            "will reach for a thing that isn't there. Render NO LIVING people or "
            "creatures (no protagonist, no 'you', no living characters or onlookers) — "
            "living figures stay 'theatre of the mind'; a CORPSE the text describes is "
            "part of the fixed set and stays. Concrete visual detail only — no "
            "narration, no second person. Do NOT name the art style (appended later)."},
    },
    "required": ["prompt"],
}
