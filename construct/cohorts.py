...
def architect_turn(provider: Provider, history: str, brief_so_far: str,
    ...
    return complete_sync(provider,
        ...
        "- set_role: they said who they want to BE — in ANY phrasing, not only "
        "first person: 'can I be the station AI', 'I'm the detective', AND the "
        "authorial third person a storyteller uses ('the main character will be "
        "the defense apprentice, and she…', 'the protagonist is a returning "
        "local'). A brief that DESCRIBES its main character has ANSWERED the "
        "who-am-I question — capture it as set_role (with the character's key "
        "traits folded in) and NEVER ask them who they'd like to be afterwards; "
        "that reads as not having listened. Capture it; make it work — if they "
        "want to play the villain or the station's AI, that just works.\n"
        ...
        "say go. THE CONFIRM HAPPENS ONCE: if the history shows they already said "
        "go (including a build that FAILED — the 'couldn't quite stabilize' line), "
        "any retry-shaped message ('keep trying', 'try again', 'go') is "
        "begin_build IMMEDIATELY — re-asking 'shall I proceed?' after they've "
        "already sent you to build reads as stalling. On a PURE 'surprise me' "
        "(nothing brought), begin_build makes the "
...
def interview_world(provider: Provider, brief: str, play_as: str = "",
    ...
    return complete_sync(provider,
        ...
        f"conclusion needs);\n"
        f"- PLACE EVERY PERSON (HARD REQUIREMENT): `person:<id> · in · place:<id>` for EACH "
        f"character — a character with no place cannot be staged, found, or chosen as the "
        f"point-of-view, so the world would be unplayable. In the OPENING scene's place put the "
        f"PROTAGONIST together with the ALLIES, WITNESSES, or companions who open the story "
        f"ALONGSIDE them (the people physically present at the start). Place ANTAGONISTS, rivals, "
        f"and anyone the protagonist must SEEK OUT or travel to in THEIR OWN location ELSEWHERE — "
        f"NOT in the opening scene (the player should not start in the same room as the villain "
        f"unless the story opens on that confrontation). (`in` is containment — a person is `in` "
        f"a place, an object is `in` a place or a person.)\n"
        f"- a few objects (each `obj:<id> · in · place:<id>` or `· in · person:<id>`) and the "
        f"opening situation as plain STATE;\n"
...
RENDER_STYLE = (
    ...
    "into something ominous or ALIVE ('the threads began speaking'). DESCRIBED ONCE, IT STAYS: "
    "within an unbroken stay in a scene, everything described — furniture, a character's posture, "
    "where someone stands — is ESTABLISHED and persists silently; re-mention it ONLY when its "
    "status changes, someone interacts with it, the current action passes through its space, or "
    "the player deliberately looks again — and a deliberate second look ADDS a genuinely new "
    "angle or meaning, never a replay of the inventory. An unchanged, unexamined scene earns no "
    "fresh inventory; silence maintains presence, and repeating an unchanged detail falsely "
    "signals change (the shawl still half-gathered three turns running reads as the world stuck, "
    "not alive). ON RE-ENTRY the eye sweeps again: returning to a place, re-ground with its main "
    "strokes — the anchors attention would find — as a reminder, never the full first-visit "
    "inventory; and the fresh look may reveal a detail newly legible because of what has since "
    "changed or been learned. "
    "ANSWER WHAT THEY ASKED — not "
