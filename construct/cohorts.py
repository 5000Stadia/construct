...
RENDER_STYLE = (
    ...
    "status changes, someone interacts with it, or the current action passes through its space. "
    "An unchanged scene earns no fresh inventory; silence maintains presence, and repeating an "
    "unchanged detail falsely signals change (the shawl still half-gathered three turns running "
    "reads as the world stuck, not alive). ON RE-ENTRY the eye sweeps again: returning to a "
    "place, re-ground with its main strokes — the anchors attention would find — as a reminder, "
    "never the full first-visit inventory; and a second look MAY go deeper: a newfound angle, a "
    "detail that means something now, a thought the character couldn't have had before. "
