...
def interview_world(provider: Provider, brief: str, play_as: str = "") -> dict:
    ...
    return complete_sync(provider,
        ...
        f"point-of-view, so the world would be unplayable. Put the PROTAGONIST and the people who "
        f"open the story with them TOGETHER in the OPENING scene's place; locate the others where "
        f"they are. (`in` is containment — a person is `in` a place, an object is `in` a place or "
        f"a person.)\n"
